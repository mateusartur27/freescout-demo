...
$lastName  = isset($parts[1]) ? mb_convert_case($parts[1], MB_CASE_TITLE, 'UTF-8') : 'Admin';
...
$email        = strtolower($firstName) . '.' . strtolower(str_replace(' ', '', $lastName)) . '@admin.local';
...
if (!$userId) {
    ...
    $stmt = $pdo->prepare(
        "INSERT INTO users
         ...
         VALUES
            (?, ?, ?, ?, 2, 'America/Sao_Paulo', 1, 1, 2, true, false, 1, ?, ?)
         RETURNING id"
    );
    ...
    // Pastas pessoais Mine (20) e Starred (25)
    $stmt = $pdo->prepare(
